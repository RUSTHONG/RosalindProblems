...
Two examples are the finite sequence (π,−2^(1/2),0,π) and the infinite sequence of odd numbers (1,3,5,7,9,…). We use the notation an to 

represent the n-th term of a sequence. 

A recurrence relation（复发关系） is a way of defining the terms of a sequence with respect to the values of previous terms. 
...
As a result, if Fn represents the number of rabbit pairs alive after the n-th month, then we obtain the Fibonacci sequence having terms Fn 
...
When finding the n-th term of a sequence defined by a recurrence relation, we can simply use the recurrence relation to generate terms for 

progressively larger values of n. This problem introduces us to the computational technique of dynamic programming, which successively builds up 
...
if __name__ == "__main__":
    totalNum = list(fibonacciYieldTool(30, 3))[-1]
    print(totalNum)
