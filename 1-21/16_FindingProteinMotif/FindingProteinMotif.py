...
def read_uniport_id(filename):
    file = open(filename, "r").read()
    uniport_id = file.split()
    return uniport_id
...
def parse_fasta(response):
    fastaList = response.read().decode("utf-8").split("\n")
    del fastaList[0], fastaList[-1]
    fastaString = ("").join(fastaList)
    return fastaString


def match(fastaString):
    for i in range(len(fastaString)-3):
        if fastaString[i] != "N":
            continue
        if fastaString[i+1] == "P":
            continue
        if fastaString[i+2] not in ["S", "T"]:
            continue
        if fastaString[i+3] == "P":
            continue
        yield i+1

uniport_id = read_uniport_id('./rosalind_mprt.txt')
for id in uniport_id:
    response = fetch_fasta(id)
    fastaString = parse_fasta(response)
    motifPosition = list(match(fastaString))
    if motifPosition:
        print(id)
        print((" ").join(map(str, motifPosition)))
